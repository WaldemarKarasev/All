 ...
 * @return the vector
 */
vec constructDefaultVector(size_t  dim, float val)
{
 ...
 * @return the vector
 */
vec constructEmptyVector(size_t  dim)
{
 ...
 * <b>NOTE: underfined behaviour when dim >> num_of_args example: vec v = newVector(100, 1.f);
 * @param dim the dimension of the vector
 * @param ... the values to put in the vector
 * @return the vector
 */
vec newVector(size_t  dim, ...)
{
    vec ret = allocateVec(dim);
