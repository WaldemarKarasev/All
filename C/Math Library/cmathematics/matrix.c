 ...
 * @return the matrix
 */
mat allocateMat(size_t rows, size_t cols)
{
    mat ret;
 ...
 * @return the matrix
 */
mat zeroMatrix(size_t rows, size_t cols)
{
 ...
 * @return the matrix
 */
mat newMatrix(size_t rows, size_t cols, size_t numVals, ...)
{
    mat ret = allocateMat(rows, cols);
 ...
 * @return the matrix
 */
mat newColMat(size_t rows, size_t cols, ...)
{
    mat ret = allocateMat(rows, cols);
 ...
 * @return the matrix
 */
mat newRowMat(size_t rows, size_t cols, ...)
{
    mat ret = allocateMat(rows, cols);
 ...
 */
vec getMatRow(mat *m, size_t row)
{
 ...
 */
vec getMatCol(mat *m, size_t col)
{
 ...
 */
bool swapRows(mat *m, size_t r1, size_t r2)
{
    r1--;
    r2--;
 ...
 * @return true if the row indices are in bounds and are unique
 */
bool addRows(mat *m, size_t r1, size_t r2)
{
    r1--;
    r2--;
 ...
 */
bool multiplyRow(mat *m, size_t r, float k)
{
    r--;
 ...
 */
bool addMultiple(mat *m, size_t r1, size_t r2, float k)
{
    r1--;
    r2--;
...
void ref(mat *m)
{
    ...
    for (size_t c = 0; c < m->cols; c++)
    {
        ...
        for (size_t col = c; col < m->cols; col++)
        {
 ...
 */
mat spliceMat(mat *m, size_t exclRow, size_t exclCol)
{
    exclRow--;
    exclCol--;
...
//     // initialize array
//     size_t *skipCols = malloc(m->cols * sizeof(unsigned int));
//     size_t noSkipCols = 0;
...
//  */
// float cofactor(mat *m, size_t r, size_t c)
// {
//     // must be a square matrix with at least 1 row
//     if (m->rows != m->cols || m->rows == 0)
//     {
